...
func ErrorWithDetail(ctx *gin.Context, code int, msgKey string, err error) {
	...
	if msgKey == constant.ErrTypeInternalServer {
		...
		case errors.Is(err, constant.ErrRecordExist):
			res.Message = i18n.GetMsgWithMap(ctx, "ErrRecordExist", nil)
		case errors.Is(constant.ErrRecordNotFound, err):
			res.Message = i18n.GetMsgWithMap(ctx, "ErrRecordNotFound", nil)
		case errors.Is(constant.ErrInvalidParams, err):
			res.Message = i18n.GetMsgWithMap(ctx, "ErrInvalidParams", nil)
		case errors.Is(constant.ErrStructTransform, err):
			res.Message = i18n.GetMsgWithMap(ctx, "ErrStructTransform", map[string]interface{}{"detail": err})
		...
		case errors.Is(constant.ErrInitialPassword, err):
			res.Message = i18n.GetMsgWithMap(ctx, "ErrInitialPassword", map[string]interface{}{"detail": err})
		case errors.As(err, &buserr.BusinessError{}):
			res.Message = err.Error()
		default:
			res.Message = i18n.GetMsgWithMap(ctx, msgKey, map[string]interface{}{"detail": err})
		}
	} else {
		res.Message = i18n.GetMsgWithMap(ctx, msgKey, map[string]interface{}{"detail": err})
	}
